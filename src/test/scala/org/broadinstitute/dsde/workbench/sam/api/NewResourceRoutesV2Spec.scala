...
class NewResourceRoutesV2Spec extends AnyFlatSpec with Matchers with ScalatestRouteTest with MockitoSugar with TestSupport {
  ...
  "GET /api/resources/v2/filter" should "correctly parse query parameters" in {
    ...
    // Act and Assert
    Get(s"/api/resources/v3/filter", SamUserRegistrationRequest(userAttributesRequest)) ~> samRoutes.route ~> check {
    ...
    Get(
      s"/api/resources/v3/filter?resourceTypes=fooType,barType&policies=fooPolicy&roles=fooRole,barRole,bazRole&actions=fooAction,barAction&includePublic=true",
