...
class GoogleExtensionRoutesSpec extends FlatSpec with Matchers with ScalatestRouteTest {
  ...
  def withDefaultRoutes[T](testCode: TestSamRoutes => T): T = {
    ...
    val samRoutes = new TestSamRoutes(null, new UserService(directoryDAO, googleExt, "dev.test.firecloud.org"), new StatusService(directoryDAO, NoExtensions), UserInfo("", defaultUserId, defaultUserEmail, 0)) with GoogleExtensionRoutes {
  ...
  "POST /api/google/policy/sync/{resourceTypeName}/{resourceId}/{accessPolicyName}" should "204 Create Google group for policy" in {
    ...
    val samRoutes = new TestSamRoutes(mockResourceService, new UserService(directoryDAO, googleExt, "dev.test.firecloud.org"), new StatusService(directoryDAO, NoExtensions), UserInfo("", defaultUserInfo.userId, defaultUserInfo.userEmail, 0)) with GoogleExtensionRoutes {
      val googleExtensions = googleExt
    }
