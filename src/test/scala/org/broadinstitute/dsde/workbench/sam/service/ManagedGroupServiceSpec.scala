...
class ManagedGroupServiceSpec extends FlatSpec with Matchers with TestSupport with MockitoSugar
  ...
  "ManagedGroupService listGroups" should "return the list of groups that passed user belongs to" in {
    ...
    val user1ExpectedMember = user1Memberships.map(s => ManagedGroupMembershipEntry(ResourceId(s), ManagedGroupService.adminPolicyName, WorkbenchEmail(s"$s@example.com")))
    ...
    val user2ExpectedMember = user2Memberships.map(s => ManagedGroupMembershipEntry(ResourceId(s), ManagedGroupService.adminPolicyName, WorkbenchEmail(s"$s@example.com")))
