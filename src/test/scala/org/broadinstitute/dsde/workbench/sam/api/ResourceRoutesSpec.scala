...
class ResourceRoutesSpec extends FlatSpec with Matchers with ScalatestRouteTest with TestSupport with AppendedClues {
  ...
  private object SamResourceActionPatterns {
    ...
    val testAnyActionAccess = ResourceActionPattern("test_any_action_access", "", false)
  ...
  private def createSamRoutes(resourceTypes: Map[ResourceTypeName, ResourceType], userInfo: UserInfo = defaultUserInfo) = {
    ...
    mockUserService.createUser(
      CreateWorkbenchUser(defaultUserInfo.userId, defaultGoogleSubjectId, defaultUserInfo.userEmail, None))
  ...
  "GET /api/resource/{resourceType}/{resourceId}/action/{action}/userEmail/{userEmail}" should "200 if user have read_policies permission" in {
  ...
  it should "400 when auth domain group exists but requesting user is not in that group" in {
    ...
    runAndWait(samRoutes.userService.createUser(
      CreateWorkbenchUser(otherUser.userId, defaultGoogleSubjectId, otherUser.userEmail, None)))
  ...
  it should "404 when creating a policy on a resource that the user doesnt have permission to see" in {
    ...
    policyDao.createResource(Resource(ResourceTypeName("rt"), ResourceId("foo"), Set.empty)).unsafeRunSync()
    val members = AccessPolicyMembership(Set(WorkbenchEmail("foo@bar.baz")), Set(ResourceAction("can_compute")), Set.empty)
  ...
  it should "404 when deleting a resource that exists but can't be seen by the user" in {
    ...
    runAndWait(samRoutes.userService.createUser(
      CreateWorkbenchUser(WorkbenchUserId(""), defaultGoogleSubjectId, WorkbenchEmail("user2@example.com"), None)))
  ...
  it should "404 adding without any access" in {
    ...
    runAndWait(samRoutes.userService.createUser(CreateWorkbenchUser(testUser.userId, defaultGoogleSubjectId, testUser.userEmail, None)))
  ...
  "DELETE /api/resource/{resourceType}/{resourceId}/policies/{policyName}/memberEmails/{email}" should "204 deleting a member" in {
    ...
    runAndWait(samRoutes.resourceService.addSubjectToPolicy(
      ...
      FullyQualifiedResourceId(resourceType.name,  ResourceId("foo")), AccessPolicyName(resourceType.ownerRoleName.value)), defaultTestUser.id))
