  ...
  "PostgresAccessPolicyDAO" - {
      ...
      "lists all a user's resources of a type when granted access via nested roles" in {
        ...
        val groupAccessChildResource =
          createResourceHierarchy(
            Option(parentGroup.id),
            Set(writeAction),
            Set(includesRole.roleName, descendsRole.roleName),
            false,
            nestedResourceType.name
          )._1

        // kitchenSink has inherited, direct and public-direct-via-group roles
        val kitchenSink =
          createResourceHierarchy(Option(user.id), Set.empty, Set(includesRole.roleName, descendsRole.roleName), false, nestedResourceType.name)._1
        val directProbePolicy = AccessPolicy(
          FullyQualifiedPolicyId(kitchenSink.fullyQualifiedId, AccessPolicyName(uuid)),
          Set(user.id),
          WorkbenchEmail(s"${uuid}@policy.com"),
    ...
    "filterResources" - {
      ...
      "filters the user's resources by policy, action, and role" in {
        ...
        val readActions =
          dao.filterResources(user.id, Set(resourceType.name), Set.empty, Set.empty, Set(readAction), Set.empty, false, samRequestContext).unsafeRunSync()
        ...
        val readerRoles =
          dao
            .filterResources(user.id, Set(resourceType.name), Set.empty, Set(readerRole.roleName), Set.empty, Set.empty, false, samRequestContext)
            .unsafeRunSync()
        ...
        val policies = dao
          .filterResources(
            ...
            Set(directProbePolicy.id.accessPolicyName),
            Set.empty,
            Set.empty,
            Set.empty,
            false,
        ...
        val readerRolesIncludingPublic =
          dao
            .filterResources(user.id, Set(resourceType.name), Set.empty, Set(readerRole.roleName), Set.empty, Set.empty, true, samRequestContext)
            .unsafeRunSync()
      ...
      "filters the user's resources by policy, action, and role with parent resources" in {
        ...
        val resourceWithParent1 = dao
          .filterResources(
            ...
            Set.empty,
            Set.empty,
            Set(parentChildResource1._2.fullyQualifiedId),
            false,
            samRequestContext
          )
          .unsafeRunSync()
        val resourceWithParent2 = dao
          .filterResources(
            user.id,
            Set(resourceType.name),
            Set.empty,
            Set.empty,
            Set.empty,
            Set(parentChildResource2._2.fullyQualifiedId),
            false,
            samRequestContext
          )
          .unsafeRunSync()
        ...
        val resourceWithParentNoAction = dao
          .filterResources(
            user.id,
            Set(resourceType.name),
            Set.empty,
        ...
        val resourceWithParentWithAction = dao
          .filterResources(
            user.id,
            Set(resourceType.name),
            Set.empty,
        ...
        val resourceWithParentNoRole = dao
          .filterResources(
            user.id,
            Set(resourceType.name),
            Set.empty,
        ...
        val resourceWithParentWithRole = dao
          .filterResources(
            user.id,
            Set(resourceType.name),
            Set.empty,
      ...
      "filters on the user's policies, roles, and actions when using nested roles" in {
        ...
        val nestedReaderRoles =
          dao
            .filterResources(user.id, Set(nestedResourceType.name), Set.empty, Set(readerRole.roleName), Set.empty, Set.empty, false, samRequestContext)
            .unsafeRunSync()
        ...
        val publicNestedReaderRoles =
          dao
            .filterResources(user.id, Set(nestedResourceType.name), Set.empty, Set(readerRole.roleName), Set.empty, Set.empty, true, samRequestContext)
            .unsafeRunSync()
