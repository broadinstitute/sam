...
trait UserRoutesSpecHelper extends FlatSpec with Matchers with ScalatestRouteTest with MockitoSugar with TestSupport{
  ...
  def withAdminRoutes[T](testCode: (TestSamRoutes, TestSamRoutes) => T): T = {
    ...
    val samRoutes = new TestSamRoutes(null, null, UserService(directoryDAO, cloudExtensions), StatusService(directoryDAO, NoExtensions), null, UserInfo(OAuth2BearerToken(""), defaultUserId, defaultUserEmail, 0), directoryDAO, cloudExtensions)
    val adminRoutes = new TestSamRoutes(null, null, UserService(directoryDAO, cloudExtensions), StatusService(directoryDAO, NoExtensions), null, UserInfo(OAuth2BearerToken(""), adminUserId, adminUserEmail, 0), directoryDAO, cloudExtensions)
  ...
  def withDefaultRoutes[T](testCode: TestSamRoutes => T): T = {
    ...
    val samRoutes = new TestSamRoutes(null, null, UserService(directoryDAO, NoExtensions), StatusService(directoryDAO, NoExtensions), null, UserInfo(OAuth2BearerToken(""), defaultUserId, defaultUserEmail, 0), directoryDAO)
    testCode(samRoutes)
  }
}