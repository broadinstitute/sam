...
class PolicyEvaluatorServiceSpec extends AsyncFlatSpec with Matchers with TestSupport {
  ...
  "hasPermission" should "return false if given action is not allowed for a user with turbo mode" in {
    val user = genUserInfo.sample.get
    val samplePolicy = genPolicy.sample.get
    val action = ResourceAction("weirdAction")
    val resource = genResource.sample.get.copy(resourceTypeName = defaultResourceType.name)
    ...
    val res = for{
      ...
      _ <- policyDAO.createResource(resource)
      _ <- policyDAO.createPolicy(policy)
      r <- service.policyEvaluatorService.hasPermission(policy.id.resource, action, user.userId, true)
    } yield {
      r shouldBe false
    }
  ...
  it should "return true if given action is on a policy directly for a direct member of the policy in turbo mode" in {
    val user = genUserInfo.sample.get
    val samplePolicy = genPolicy.sample.get
  ...
  it should "return true if given action is allowed via a role for a direct member of the policy in turbo mode" in {
    val user = genUserInfo.sample.get
    val samplePolicy = genPolicy.sample.get
...
//  it should "return true if given action is allowed via a role for an indirect member of the policy in turbo mode" in {
//    val user = genUserInfo.sample.get
//    val managedGroup = BasicWorkbenchGroup(genWorkbenchGroupName.sample.get, Set(user.userId), genNonPetEmail.sample.get)
//    val samplePolicy = genPolicy.sample.get
//    val sampleRole = defaultResourceType.roles.head.roleName
//    val action = defaultResourceType.roles.head.actions.head
//
//    val resource = genResource.sample.get.copy(authDomain = Set.empty, resourceTypeName = defaultResourceType.name)
//    val policyWithGroup = AccessPolicy.members.modify(_ + managedGroup.id)(samplePolicy)
//
//    val policyWithRole = AccessPolicy.roles.modify(_ + sampleRole)(policyWithGroup)
//    val policy = SamLenses.resourceIdentityAccessPolicy.set(resource.fullyQualifiedId)(policyWithRole)
//
//    val res = for{
//      _ <- setup()
...
//  }
  ...
  it should "return false if given action is not present for a user via a role in turbo mode" in {
    val user = genUserInfo.sample.get
    val samplePolicy = genPolicy.sample.get
