...
object TestSamRoutes {
  ...
  def apply(resourceTypes: Map[ResourceTypeName, ResourceType],
    ...
    new TestSamRoutes(mockResourceService, policyEvaluatorService, mockUserService, mockStatusService, mockManagedGroupService, user, directoryDAO, registrationDAO, tosService = mockTosService, cloudExtensions = cloudXtns, azureRoutes = azureRoutes)
  }
}
