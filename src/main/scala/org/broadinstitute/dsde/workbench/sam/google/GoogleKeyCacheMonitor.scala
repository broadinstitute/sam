...
object GoogleKeyCacheMonitorSupervisor {
  ...
  def props(pollInterval: FiniteDuration, pollIntervalJitter: FiniteDuration, pubSubDao: GooglePubSubDAO, pubSubTopicName: String, pubSubSubscriptionName: String, workerCount: Int, googleKeyCache: GoogleKeyCache)(implicit executionContext: ExecutionContext): Props = {
    Props(new GoogleKeyCacheMonitorSupervisor(pollInterval, pollIntervalJitter, pubSubDao, pubSubTopicName, pubSubSubscriptionName, workerCount, googleKeyCache))
  }
}

class GoogleKeyCacheMonitorSupervisor(val pollInterval: FiniteDuration, pollIntervalJitter: FiniteDuration, pubSubDao: GooglePubSubDAO, pubSubTopicName: String, pubSubSubscriptionName: String, workerCount: Int, googleKeyCache: GoogleKeyCache)(implicit executionContext: ExecutionContext) extends Actor with LazyLogging {
  ...
  def init = {
    for {
      ...
      //also need to add the sam service account as a publisher to this topic
      //also need to register the object change notifications to publish to pubsub
    } yield Start
  }

  def startOne(): Unit = {
    logger.info("starting GoogleKeyCacheMonitorActor")
    actorOf(GoogleKeyCacheMonitor.props(pollInterval, pollIntervalJitter, pubSubDao, pubSubSubscriptionName, googleKeyCache))
  }
...
object GoogleKeyCacheMonitor {
  ...
  def props(pollInterval: FiniteDuration, pollIntervalJitter: FiniteDuration, pubSubDao: GooglePubSubDAO, pubSubSubscriptionName: String, googleKeyCache: GoogleKeyCache)(implicit executionContext: ExecutionContext): Props = {
    Props(new GoogleKeyCacheMonitorActor(pollInterval, pollIntervalJitter, pubSubDao, pubSubSubscriptionName, googleKeyCache))
  }
}

class GoogleKeyCacheMonitorActor(val pollInterval: FiniteDuration, pollIntervalJitter: FiniteDuration, pubSubDao: GooglePubSubDAO, pubSubSubscriptionName: String, googleKeyCache: GoogleKeyCache)(implicit executionContext: ExecutionContext) extends Actor with LazyLogging with FutureSupport {
  ...
  override def receive = {
    ...
    case Some(message: PubSubMessage) =>
      ...
      val (pet, keyId): (PetServiceAccount, ServiceAccountKeyId) = parseMessage(message)

      googleKeyCache.removeKey(pet, keyId)
