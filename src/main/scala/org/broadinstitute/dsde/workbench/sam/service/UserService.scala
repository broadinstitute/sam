...
/**
  ...
  */
class UserService(val directoryDAO: DirectoryDAO, val cloudExtensions: CloudExtensions, val registrationDAO: RegistrationDAO, domainBlacklist: Seq[String])(implicit val executionContext: ExecutionContext, contextShift: ContextShift[IO]) extends LazyLogging {

  def createUser(user: CreateWorkbenchUser, samRequestContext: SamRequestContext): Future[UserStatus] = {
    for {
      _ <- UserService.validateEmailAddress(user.email, domainBlacklist).unsafeToFuture()
  ...
  def inviteUser(invitee: InviteUser, samRequestContext: SamRequestContext): IO[UserStatusDetails] =
    for {
      _ <- UserService.validateEmailAddress(invitee.inviteeEmail, domainBlacklist)
    ...
    directoryDAO.loadUser(userId, samRequestContext).unsafeToFuture().flatMap {
      case Some(user) if !userDetailsOnly =>
        for {
          ...
          allUsersGroup <- cloudExtensions.getOrCreateAllUsersGroup(directoryDAO, samRequestContext)
          allUsersStatus <- directoryDAO.isGroupMember(allUsersGroup.id, user.id, samRequestContext).unsafeToFuture() recover { case e: NameNotFoundException => false }
...
object UserService {
  ...
  def validateEmailAddress(email: WorkbenchEmail, domainBlacklist: Seq[String]): IO[Unit] = {
    email.value match {
      case emailString if domainBlacklist.exists(domain => emailString.endsWith("@" + domain)) => IO.raiseError(new WorkbenchExceptionWithErrorReport(ErrorReport(StatusCodes.BadRequest, s"email domain not permitted [${email.value}]")))
