    ...
    resourceTypes.get(SamResourceTypes.resourceTypeAdminName) match {
      ...
      case Some(resourceTypeAdmin) =>
        for {
          ...
          createdAdminType <- createResourceType(resourceTypeAdmin, SamRequestContext(null)) // todo: this is init on boot -- keep as null?
          ...
          result <- resourceTypes.values.filterNot(_.name == SamResourceTypes.resourceTypeAdminName).toList.traverse { rt =>
            for {
              _ <- createResourceType(rt, SamRequestContext(null)) // todo: this is init on boot -- keep as null?
              ...
              _ <- persistResource(resourceTypeAdmin, ResourceId(rt.name.value), Set(policy), Set.empty, SamRequestContext(null)).recover { // todo: this is init on boot -- keep as null?
