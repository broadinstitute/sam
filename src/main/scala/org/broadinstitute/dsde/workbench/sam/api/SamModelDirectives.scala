...
trait SamModelDirectives {
  ...
  def withSubject(email: WorkbenchEmail, parentSpan: Span = null): Directive1[WorkbenchSubject] = //todo: create a root span here instead of allowing null?
    onSuccess(userService.getSubjectFromEmail(email, parentSpan)).map {
