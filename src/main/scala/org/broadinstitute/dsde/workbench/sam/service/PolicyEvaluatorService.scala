...
class PolicyEvaluatorService(
  ...
  def hasPermission(resource: FullyQualifiedResourceId, action: ResourceAction, userId: WorkbenchUserId, traceContext: TraceContext): IO[Boolean] = {
    // first attempt the shallow check and fallback to the full check if it returns false
    for {
      attempt1 <- traceIOWithContext("shallowCheck", traceContext)(_ => hasPermissionShallowCheck(resource, action, userId, traceContext))
      attempt2 <- if (attempt1) IO.pure(attempt1) else traceIOWithContext("fullCheck", traceContext)(_ => hasPermissionFullCheck(resource, action, userId, traceContext))
    } yield {
      attempt2
    }
  }

  def hasPermissionFullCheck(resource: FullyQualifiedResourceId, action: ResourceAction, userId: WorkbenchUserId, traceContext: TraceContext): IO[Boolean] = {
    def checkPermission(force: Boolean) =
      listUserResourceActions(resource, userId, force, traceContext).map { _.contains(action) }
    ...
    for {
      attempt1 <- traceIOWithContext("checkWithCache", traceContext)(_ => checkPermission(force = false))
      attempt2 <- if (attempt1) IO.pure(attempt1) else traceIOWithContext("checkWithoutCache", traceContext)(_ =>checkPermission(force = true))
    } yield {
      attempt2
    }
  }
  ...
  /**
    ...
    */
  def hasPermissionShallowCheck(resource: FullyQualifiedResourceId, action: ResourceAction, userId: WorkbenchUserId, traceContext: TraceContext): IO[Boolean] = {
    ...
    for {
      res <- resourceTypes.get(resource.resourceTypeName).traverse {
        rt =>
          ...
          val hasAction = directMemberHasActionOnResource(resource, roleNamesWithAction, action, userId, traceContext)
          ...
          if (!rt.isAuthDomainConstrainable || !rt.actionPatterns.exists( ap => ap.authDomainConstrainable && ap.matches(action)) ) {
            ...
            for {
              ...
              authDomainsResult <- accessPolicyDAO.loadResourceAuthDomain(resource, traceContext)
              authConstraintOk <- authDomainsResult match {
                ...
                case LoadResourceAuthDomainResult.Constrained(authDomains) =>
                  ...
                  if (authDomains.size > 1) {
                    ...
                    directoryDAO.isGroupMember(authDomains.head, userId, traceContext)
                  }
              }
  ...
  private def directMemberHasActionOnResource(resource: FullyQualifiedResourceId, roleNamesWithAction: Set[ResourceRoleName], action: ResourceAction, userId: WorkbenchUserId, traceContext: TraceContext): IO[Boolean] = {
    for {
      // get the policies for this resource and filter to ones that contain the roles with the action, or the action directly
      policies <- accessPolicyDAO.listAccessPolicies(resource, traceContext).map(_.toList.filter(p => p.roles.intersect(roleNamesWithAction).nonEmpty || p.actions.contains(action)))
  ...
  /**
    ...
    * @return
    */
  def listUserResourceActions(resource: FullyQualifiedResourceId, userId: WorkbenchUserId, force: Boolean = false, traceContext: TraceContext): IO[Set[ResourceAction]] = {
    ...
    for {
      _ <- if (force) accessPolicyDAO.evictIsMemberOfCache(userId, traceContext) else IO.unit
      ...
      policiesForResource <- listResourceAccessPoliciesForUser(resource, userId, traceContext)
      ...
      res <- if (isConstrainable) {
        for {
          ...
          policyActions <- authDomainsResult match {
            case LoadResourceAuthDomainResult.Constrained(authDomains) =>
              listUserManagedGroups(userId, traceContext).map{
  ...
  def listUserAccessPolicies(resourceTypeName: ResourceTypeName, userId: WorkbenchUserId, traceContext: TraceContext): IO[Set[UserPolicyResponse]] =
    for {
      rt <- IO.fromEither(
        resourceTypes
      ...
      ridAndPolicyName <- accessPolicyDAO.listAccessPolicies(resourceTypeName, userId, traceContext) // List all policies of a given resourceType the user is a member of
      rids = ridAndPolicyName.map(_.resourceId)

      resources <- if (isConstrained) accessPolicyDAO.listResourcesWithAuthdomains(resourceTypeName, rids, traceContext)
      else IO.pure(Set.empty)
      authDomainMap = resources.map(x => x.resourceId -> x.authDomain).toMap

      userManagedGroups <- if (isConstrained) listUserManagedGroups(userId, traceContext) else IO.pure(Set.empty[WorkbenchGroupName])
      ...
      publicPolicies <- accessPolicyDAO.listPublicAccessPolicies(resourceTypeName, traceContext)
  ...
  def listUserManagedGroupsWithRole(userId: WorkbenchUserId, traceContext: TraceContext): IO[Set[ManagedGroupAndRole]] =
    for {
      ripns <- accessPolicyDAO.listAccessPolicies(ManagedGroupService.managedGroupTypeName, userId, traceContext)
    } yield {
      ripns
  ...
  def listUserManagedGroups(userId: WorkbenchUserId, traceContext: TraceContext): IO[Set[WorkbenchGroupName]] =
    for {
      policies <- listUserManagedGroupsWithRole(userId, traceContext)
    } yield {
      policies.map(_.groupName)
    }

  def listResourceAccessPoliciesForUser(resource: FullyQualifiedResourceId, userId: WorkbenchUserId, traceContext: TraceContext): IO[Set[AccessPolicyWithoutMembers]] =
    for {
      policies <- traceIOWithContext("listAccessPoliciesForUser", traceContext)(_ => accessPolicyDAO.listAccessPoliciesForUser(resource, userId, traceContext))
      publicPolicies <- traceIOWithContext("listPublicAccessPolicies", traceContext)(_ => accessPolicyDAO.listPublicAccessPolicies(resource, traceContext))
