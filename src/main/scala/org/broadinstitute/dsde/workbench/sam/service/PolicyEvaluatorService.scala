...
class PolicyEvaluatorService(
    ...
    extends LazyLogging {
  def initPolicy(parentSpan: Span = null): IO[Unit] = {
  ...
  def hasPermission(resource: FullyQualifiedResourceId, action: ResourceAction, userId: WorkbenchUserId, parentSpan: Span): IO[Boolean] = {
    // first attempt the shallow check and fallback to the full check if it returns false
    for {
      attempt1 <- traceIOWithParent("shallowCheck", parentSpan)(_ => hasPermissionShallowCheck(resource, action, userId, parentSpan))
      attempt2 <- if (attempt1) IO.pure(attempt1) else traceIOWithParent("fullCheck", parentSpan)(_ => hasPermissionFullCheck(resource, action, userId, parentSpan))
    } yield {
      attempt2
    }
  }

  def hasPermissionFullCheck(resource: FullyQualifiedResourceId, action: ResourceAction, userId: WorkbenchUserId, parentSpan: Span): IO[Boolean] = {
  ...
  /**
    ...
    */
  def hasPermissionShallowCheck(resource: FullyQualifiedResourceId, action: ResourceAction, userId: WorkbenchUserId, parentSpan: Span): IO[Boolean] = {
    ...
    for {
      res <- resourceTypes.get(resource.resourceTypeName).traverse {
        rt =>
          ...
          val hasAction = directMemberHasActionOnResource(resource, roleNamesWithAction, action, userId, parentSpan)
          ...
          if (!rt.isAuthDomainConstrainable || !rt.actionPatterns.exists( ap => ap.authDomainConstrainable && ap.matches(action)) ) {
            ...
            for {
              ...
              authDomainsResult <- accessPolicyDAO.loadResourceAuthDomain(resource, parentSpan)
              authConstraintOk <- authDomainsResult match {
                ...
                case LoadResourceAuthDomainResult.Constrained(authDomains) =>
                  ...
                  if (authDomains.size > 1) {
                    ...
                    directoryDAO.isGroupMember(authDomains.head, userId, parentSpan)
                  }
              }
  ...
  private def directMemberHasActionOnResource(resource: FullyQualifiedResourceId, roleNamesWithAction: Set[ResourceRoleName], action: ResourceAction, userId: WorkbenchUserId, parentSpan: Span): IO[Boolean] = {
    for {
      // get the policies for this resource and filter to ones that contain the roles with the action, or the action directly
      policies <- accessPolicyDAO.listAccessPolicies(resource, parentSpan).map(_.toList.filter(p => p.roles.intersect(roleNamesWithAction).nonEmpty || p.actions.contains(action)))
  ...
  /**
    ...
    * @return
    */
  def listUserResourceActions(resource: FullyQualifiedResourceId, userId: WorkbenchUserId, force: Boolean = false, parentSpan: Span): IO[Set[ResourceAction]] = {
    ...
    for {
      _ <- if (force) accessPolicyDAO.evictIsMemberOfCache(userId, parentSpan) else IO.unit
      ...
      res <- if (isConstrainable) {
        for {
          ...
          policyActions <- authDomainsResult match {
            case LoadResourceAuthDomainResult.Constrained(authDomains) =>
              listUserManagedGroups(userId, parentSpan).map{
  ...
  def listUserAccessPolicies(resourceTypeName: ResourceTypeName, userId: WorkbenchUserId, parentSpan: Span): IO[Set[UserPolicyResponse]] =
    for {
      rt <- IO.fromEither(
        resourceTypes
      ...
      resources <- if (isConstrained) accessPolicyDAO.listResourcesWithAuthdomains(resourceTypeName, rids, parentSpan)
      else IO.pure(Set.empty)
      authDomainMap = resources.map(x => x.resourceId -> x.authDomain).toMap

      userManagedGroups <- if (isConstrained) listUserManagedGroups(userId, parentSpan) else IO.pure(Set.empty[WorkbenchGroupName])
      ...
      publicPolicies <- accessPolicyDAO.listPublicAccessPolicies(resourceTypeName, parentSpan)
  ...
  def listUserManagedGroupsWithRole(userId: WorkbenchUserId, parentSpan: Span): IO[Set[ManagedGroupAndRole]] =
    for {
  ...
  def listUserManagedGroups(userId: WorkbenchUserId, parentSpan: Span): IO[Set[WorkbenchGroupName]] =
    for {
      policies <- listUserManagedGroupsWithRole(userId, parentSpan)
    } yield {
      policies.map(_.groupName)
    }

  def listResourceAccessPoliciesForUser(resource: FullyQualifiedResourceId, userId: WorkbenchUserId, parentSpan: Span): IO[Set[AccessPolicyWithoutMembers]] =
    for {
      policies <- traceIOWithParent("listAccessPoliciesForUser", parentSpan)(_ => accessPolicyDAO.listAccessPoliciesForUser(resource, userId, parentSpan))
      publicPolicies <- traceIOWithParent("listPublicAccessPolicies", parentSpan)(_ => accessPolicyDAO.listPublicAccessPolicies(resource, parentSpan))
