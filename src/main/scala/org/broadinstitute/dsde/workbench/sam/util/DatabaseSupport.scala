...
trait DatabaseSupport {
  ...
  /**
    * Run a database transaction with isolation level set to serializable.
    * See https://www.postgresql.org/docs/9.6/transaction-iso.html.
    * Use this when you need to be sure there are no concurrent changes that affect the outcome of databaseFunction.
    * When a transaction fails due to a serialization failure it may be retried.
    *
    ...
    *
    * @param dbQueryName name used in traces
    * @param samRequestContext
    * @param maxTries how many time in total should this transaction be tried
    * @param databaseFunction function that performs the function within the transaction
    * @param timer used for scheduling retries
