...
trait SecurityDirectives {
  ...
  def requireOneOfAction(resource: FullyQualifiedResourceId, requestedActions: Set[ResourceAction], userId: WorkbenchUserId): Directive0 =
    Directives.mapInnerRoute { innerRoute =>
      onSuccess(evalAccess(resource, requestedActions, userId, innerRoute))(identity)
    }

  private def evalAccess(resource: FullyQualifiedResourceId, requestedActions: Set[ResourceAction], userId: WorkbenchUserId, innerRoute: Route): IO[Route] = {
    requestedActions.toList.traverse(action => policyEvaluatorService.hasPermission(resource, action, userId)).flatMap { hasPermissionResults =>
      if (hasPermissionResults.contains(true)) {
        IO.pure(innerRoute)
      } else {
        listActions(resource, userId, requestedActions).map { actions =>
          if (actions.isEmpty)
            Directives.failWith(
